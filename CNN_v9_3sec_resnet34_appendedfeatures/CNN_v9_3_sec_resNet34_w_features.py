...
class ImageWithUnlinkedFeaturesDataset(Dataset):
    def __init__(self, root_dir, feature_csv, transform=None):
        ...
        # Exclude the first 2 and last 1 columns (filename, duration, and label)
        self.features = df.iloc[:, 2:-1].values.astype('float32')
...
dataset = ImageWithUnlinkedFeaturesDataset(
    ...
    feature_csv='./gtzan/features_3_sec_cleaned.csv',
    transform=transform
)
...
summary(model, [(1,512,512),(1,57,1)]) # after moving, won't work before if using cuda
